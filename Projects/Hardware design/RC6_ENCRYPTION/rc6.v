...
// module for barrel 32 bit barrel shift
module rol32_barrel (
    input  wire [31:0] x,
    input  wire [4:0]  s,
    output wire [31:0] y
);
    wire [31:0] s1  = s[0] ? {x[30:0], x[31]}      : x;
    wire [31:0] s2  = s[1] ? {s1[29:0], s1[31:30]} : s1;
    wire [31:0] s4  = s[2] ? {s2[27:0], s2[31:28]} : s2;
    wire [31:0] s8  = s[3] ? {s4[23:0], s4[31:24]} : s4;
    wire [31:0] s16 = s[4] ? {s8[15:0], s8[31:16]} : s8;
    assign y = s16;
endmodule
...
// module of the cipher, encrypting using official RC6 encryption procedure
module rc6 (
    input wire clk,
    input wire reset,

    input wire [31:0] A_in, B_in, C_in, D_in,         // input wires for 16b of plain text 
    input wire [1407:0] key,                          // input wire for round keys, used instead of an array of word keys
    input wire trigger,                               // trigger wire to start the encryption process
    output wire [31:0] A_out, B_out, C_out, D_out,    // output wires for 16b of cipher text
    output reg valid                                  // wire signaling valid output
);
    
    // state of the overall encryption process
    localparam [1:0]
        IDLE    = 2'd0,         // module waits for the data and trigger
        PROCESS = 2'd1,         // processes the plain text into cipher text
        YIELD   = 2'd2,         // sends 1 to the valid wire, signaling end of encryption and valid data
        PREPARE = 2'd3;         // prepares the module for next batch of plain text to process

    reg [1:0] state;            // state reg of the state machine
    
    
    reg [31:0]  S[0:43];        // array of round keys, obrained from the key wire
    reg [31:0]  A, B, C, D;     // work registers for use in the encryption procedure
    reg [31:0]  t, u;           // help registers for the encryption procedure
    reg [5:0]   round_counter;  // counter for each rounds of encryption
    reg [1:0]   pipeline_ctr;   // counter for pipelining the steps of encryption, solving timing issue
    wire [31:0] rot_t, rot_u, rot_A, rot_C; // wires for results of barrel shifts

    integer gen_i;

    // barrel shift modules for enryption calculatiions
    rol32_barrel bs_t(
        .x(B * ((B << 1) + 1)),
        .s(5'd5),
        .y(rot_t)
            ...
            case (state)
                ...
                PROCESS: begin
                    if (round_counter == 5'd0) begin
                        ...
                        if (pipeline_ctr == 2'd0) begin
                            t <= rot_t;
                            u <= rot_u;
                            pipeline_ctr <= 2'd1;
                        end else if (pipeline_ctr == 2'd1) begin
                            A <= rot_A + S[round_counter << 1];
                            C <= rot_C + S[(round_counter << 1) + 1];
                            pipeline_ctr <= 2'd2;
                        end else begin
                            A <= B;
                            B <= C;
